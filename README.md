...
│   │   ├── user_operation.py           # 【用户接口】纯LBA操作，无需接触框架代码
...
### user_operation.py — 用户操作接口（推荐）

纯 LBA 操作文件，框架代码通过 import 复用。开箱即用：

```bash
python user_operation.py                                # 顺序填充全盘
python user_operation.py --type random                  # 随机填充全盘
python user_operation.py --type sprandom                # SPRandom 单遍预处理
python user_operation.py --type sprandom --regions 20   # SPRandom 自定义区域数
python user_operation.py --type random --seed 42        # 可复现随机填充
python user_operation.py --type seq --divide 36         # 按SB粒度分批顺序填充
```

自定义操作时，直接编辑 `user_operation.py` 中的 `user_operations()` 函数即可：

```python
# user_operation.py — 只需关注 LBA 写入逻辑
...
### ScriptBuilder 框架 — 框架层说明

`fill_sequential.py` 和 `fill_random.py` 内部封装了 `ScriptBuilder` 框架，
`user_operation.py` 是面向用户的轻量包装。框架提供以下 `ctx` 接口：
...
def my_ops(ctx, total_pages=216):
